...
function grav(o){
	...
	for(let i=0; i<b.length; i++){
		if(i!=o && !b[o].test){
			...
			let dir = new THREE.Vector3().subVectors(o1, o0).normalize();
			let distSq = o0.distanceToSquared(o1);
			force.add(dir.multiplyScalar(b[o].m*b[i].m/distSq));
		}
	}
	return force;
}
function cameraControl(){
	...
	let totalMass = 0;
	for(let i=0; i<b.length; i++){
		com.add(b[i].obj.position.clone().multiplyScalar(b[i].m));
		totalMass += b[i].m;
	}
...
function trailControl(){
	for(let i=0; i<t.length; i++){
		...
		if(t[i].pos.length > 200) t[i].pos.pop();
		let oldGeo = t[i].line.geometry;
		let newGeo = new THREE.BufferGeometry();
		newGeo.setFromPoints(t[i].pos);
		t[i].line.geometry = newGeo;
		oldGeo.dispose();
	}
}
let work = false;
...
function toggleTrails(){
	showT = !showT;
	for(let i=0; i<t.length; i++){
		t[i].line.visible = showT;
	}
}
function toggleObjects(){
	showO = !showO;
	for(let i=0; i<b.length; i++){
		b[i].obj.visible = showO;
	}
}
function toggleBkg(){
	showB = !showB;
...
function testMass(){
	...
	if(t[testIndex].pos.length > 200) t[testIndex].pos.pop();
	let oldGeo = t[testIndex].line.geometry;
	let newGeo = new THREE.BufferGeometry();
	newGeo.setFromPoints(t[testIndex].pos);
	t[testIndex].line.geometry = newGeo;
	oldGeo.dispose();
}
